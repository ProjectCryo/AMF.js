(function() {
  ...
  AMF3.STRING.decode = function() {
    ...
    str = this.readByte(header.value).toString('utf8');
  ...
  readAMF3ObjectHeader = function(flags) {
    ...
    for (x = i = 0, ref = staticKeyLen - 1; 0 <= ref ? i <= ref : i >= ref; x = 0 <= ref ? ++i : --i) {
      trait.staticFields[x] = this.deserialize(AMF3.STRING, AMF3);
    }
  ...
  AMF3.OBJECT.decode = function() {
    var header, i, len1, ref, results, ret, trait, x;
    header = this.readAMFHeader();
    if (!header.isDef) {
      return this.amf3ObjectReferences[header.value];
    }
    ...
    if (trait.externalizable) {
      ...
      return AMFDecoder.amf3Externalizables[name].read(this);
    }
    ret = new classes.Serializable(trait.name || void 0);
    ref = ret.staticFields;
    results = [];
    for (i = 0, len1 = ref.length; i < len1; i++) {
      x = ref[i];
      results.push(ret[x] = this.decode(AMF3));
    }
    return results;
  };
